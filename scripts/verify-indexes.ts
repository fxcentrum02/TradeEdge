// ===========================================
// INDEX VERIFICATION & CREATION SCRIPT
...
// Define all REQUIRED indexes (existing + new compound indexes)
...
async function main() {
    console.log('========================================');
    console.log('INDEX VERIFICATION & CREATION SCRIPT');
    ...
    for (const [collName, requiredIndexes] of Object.entries(REQUIRED_INDEXES)) {
        ...
        for (const req of requiredIndexes) {
            // Check if an index with the same key already exists (by key, not name)
            ...
            if (exists) {
                ...
                console.log(`  ❌ MISSING: ${req.name} ${JSON.stringify(req.key)}`);
    ...
    if (totalMissing === 0) {
        ...
        console.log(`\nTotal: ${totalMissing} indexes were missing, ${totalCreated} created.`);
    }

    // Now run explain() on key queries to verify IXSCAN
    console.log('\n========================================');
    console.log('EXPLAIN ANALYSIS (IXSCAN Verification)');
    console.log('========================================\n');
    ...
    // 1. User plans: ROI eligibility query
    {
        ...
        console.log(`1. ROI Eligibility Query (user_plans):`);
    ...
    // 2. Transactions: user history (userId + sort createdAt desc)
    {
        ...
        if (sampleUserId) {
            ...
            console.log(`2. Transaction History (transactions):`);
            console.log(`   Stage: ${stage}`);
            console.log(`   Index: ${indexName}`);
            console.log(`   Docs Examined: ${stats?.totalDocsExamined}`);
            console.log(`   Keys Examined: ${stats?.totalKeysExamined}`);
    ...
    // 3. Withdrawals: admin queue (status + sort createdAt desc)
    {
        const explain = await db.collection(Collections.WITHDRAWALS).find({ status: 'PENDING' }).sort({ createdAt: -1 }).limit(20).explain('executionStats') as any;
        const winningPlan = explain.queryPlanner?.winningPlan;
        const stage = getStage(winningPlan);
        const indexName = getIndexName(winningPlan);
        const stats = explain.executionStats;

        console.log(`3. Admin Withdrawal Queue (withdrawals):`);
        console.log(`   Stage: ${stage}`);
        console.log(`   Index: ${indexName}`);
        console.log(`   Docs Examined: ${stats?.totalDocsExamined}`);
        console.log(`   Keys Examined: ${stats?.totalKeysExamined}`);
        ...
        explainResults.push({ query: 'Admin Withdrawal Queue', collection: Collections.WITHDRAWALS, stage, indexName, docsExamined: stats?.totalDocsExamined || 0, keysExamined: stats?.totalKeysExamined || 0 });
    }

    // 4. Payment Tickets: admin queue (status + sort createdAt desc)
    {
        const explain = await db.collection(Collections.PAYMENT_TICKETS).find({ status: 'PENDING' }).sort({ createdAt: -1 }).limit(20).explain('executionStats') as any;
        const winningPlan = explain.queryPlanner?.winningPlan;
        const stage = getStage(winningPlan);
        const indexName = getIndexName(winningPlan);
        const stats = explain.executionStats;

        console.log(`4. Admin Payment Ticket Queue (payment_tickets):`);
        console.log(`   Stage: ${stage}`);
        console.log(`   Index: ${indexName}`);
        console.log(`   Docs Examined: ${stats?.totalDocsExamined}`);
        console.log(`   Keys Examined: ${stats?.totalKeysExamined}`);
        ...
        explainResults.push({ query: 'Admin Payment Queue', collection: Collections.PAYMENT_TICKETS, stage, indexName, docsExamined: stats?.totalDocsExamined || 0, keysExamined: stats?.totalKeysExamined || 0 });
    }

    // 5. User Plans: user plan history (userId + sort createdAt desc)
    {
        const sampleUserId = await db.collection(Collections.USERS).findOne({}, { projection: { _id: 1 } });
        if (sampleUserId) {
            const explain = await db.collection(Collections.USER_PLANS).find({ userId: sampleUserId._id }).sort({ createdAt: -1 }).explain('executionStats') as any;
            const winningPlan = explain.queryPlanner?.winningPlan;
            const stage = getStage(winningPlan);
            const indexName = getIndexName(winningPlan);
            const stats = explain.executionStats;

            console.log(`5. User Plan History (user_plans):`);
            console.log(`   Stage: ${stage}`);
            console.log(`   Index: ${indexName}`);
            console.log(`   Docs Examined: ${stats?.totalDocsExamined}`);
            console.log(`   Keys Examined: ${stats?.totalKeysExamined}`);
            ...
            explainResults.push({ query: 'User Plan History', collection: Collections.USER_PLANS, stage, indexName, docsExamined: stats?.totalDocsExamined || 0, keysExamined: stats?.totalKeysExamined || 0 });
        }
    }

    // 6. Referral earnings by tier (userId aggregation)
    {
        const sampleUserId = await db.collection(Collections.USERS).findOne({}, { projection: { _id: 1 } });
        if (sampleUserId) {
            const explain = await db.collection(Collections.REFERRAL_EARNINGS).find({ userId: sampleUserId._id }).explain('executionStats') as any;
            const winningPlan = explain.queryPlanner?.winningPlan;
            const stage = getStage(winningPlan);
            const indexName = getIndexName(winningPlan);
            const stats = explain.executionStats;

            console.log(`6. Referral Earnings by User (referral_earnings):`);
            console.log(`   Stage: ${stage}`);
            console.log(`   Index: ${indexName}`);
            console.log(`   Docs Examined: ${stats?.totalDocsExamined}`);
            console.log(`   Keys Examined: ${stats?.totalKeysExamined}`);
            ...
            explainResults.push({ query: 'Referral Earnings', collection: Collections.REFERRAL_EARNINGS, stage, indexName, docsExamined: stats?.totalDocsExamined || 0, keysExamined: stats?.totalKeysExamined || 0 });
        }
    }

    // Final Summary
    console.log('\n========================================');
    console.log('EXPLAIN RESULTS SUMMARY');
    console.log('========================================');
...
// Helper to extract the index name from the winning plan
function getIndexName(plan: any): string {
    if (!plan) return 'none';
