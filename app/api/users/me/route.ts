...
export async function GET(request: NextRequest): Promise<NextResponse<ApiResponse<UserDashboard & { activePlanDetails: any[] }>>> {
    ...
    try {
        ...
        // Define sevenDaysAgo for the sparkline query
        ...
        // MASSIVE PARALLELIZATION: Collapse 7+ sequential RTTs into 1
        const [
            activeUserPlans, 
            allPlans, 
            wallet, 
            ...
            db.collection(Collections.USER_PLANS)
                ...
                .toArray(),
            findActivePlans(),
            getWalletSummary(user._id.toString()),
            user.referredById ? findUserById(user.referredById) : Promise.resolve(null),
            db.collection(Collections.USER_PLANS)
                .aggregate([
        ...
        const parallelFetchTime = performance.now();
        ...
        // Update user.tradePower if it changed (runs in background/parallel)
        if (tradePower !== user.tradePower) {
            db.collection(Collections.USERS).updateOne(
                { _id: user._id },
                { $set: { tradePower, updatedAt: new Date() } }
            ).catch(err => console.error('Failed to update tradePower background:', err));
        }
        ...
        console.log(`[PERF] - parallelFetch: ${(parallelFetchTime - userFetchTime).toFixed(2)}ms`);
        console.log(`[PERF] - processing: ${(endTime - parallelFetchTime).toFixed(2)}ms`);
